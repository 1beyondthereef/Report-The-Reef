...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 7. Delete reservations (table may not exist — defensive)
    try {
      await adminClient.from("reservations").delete().eq("user_id", userId);
    } catch (e7) {
      console.warn("[AccountDelete] Reservations cleanup skipped (table may not exist):", e7);
    }

    // 8. Delete user-filed reports (table may not exist — defensive)
    try {
      await adminClient.from("reports").delete().eq("reporter_id", userId);
    } catch (e8) {
      console.warn("[AccountDelete] Reports cleanup skipped (table may not exist):", e8);
    }

    // 9. Remove user from conversations (nullify to avoid FK issues on profile delete)
    try {
      await adminClient.from("conversations").update({ user1_id: null }).eq("user1_id", userId);
      await adminClient.from("conversations").update({ user2_id: null }).eq("user2_id", userId);
    } catch (e9) {
      console.warn("[AccountDelete] Conversations cleanup skipped:", e9);
    }

    // 11. Delete avatar from storage (defensive — don't block on failure)
    try {
    ...
    const { error: e12 } = await adminClient
      ...
      .delete()
      .eq("id", userId);
    if (e12) throw { step: "delete_profile", detail: e12.message };

    // 13. Delete auth user
    const { error: e13 } = await adminClient.auth.admin.deleteUser(userId);
    if (e13) throw { step: "delete_auth_user", detail: e13.message };
